...
import { rehypeExpoLocalImages, type RehypeExpoLocalImagesOptions} from './plugins/rehype-expo-local-images';
...
let _compiler: ReturnType<typeof import('@mdx-js/mdx').createProcessor> | null = null;

/**
 * Create and return the singleton MDX compiler instance.
 * This is done asynchronously to use ESM code inside CJS contexts. 
 */
export async function createSingletonCompiler(options: import('@mdx-js/mdx').ProcessorOptions) {
  if (!_compiler) {
...
export function createTransformer({
  ...
  async function transformMdx(props: { filename: string; src: string }) {
    ...
    const compiler = await createSingletonCompiler({
      ...
      recmaPlugins: [
        [recmaExpoRuntime, { visit: estreeVisit }],
      ],
    });
    ...
    try {
      ...
      throw new Error(`Failed to process MDX for ${props.filename}`, { cause: error });
    }
